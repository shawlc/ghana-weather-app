...
ghanaCities.features.forEach(function(marker) {
  ...
  var tempCelsius = String(parseInt(parseFloat(marker.properties.value.main.temp) - 273.15)) + " deg Celsius"
  ...
  var iconMarker = new mapboxgl.Marker(elem)
    ...
    .setPopup(new mapboxgl.Popup({
      ...
      .setHTML('<h3>' + marker.properties.name + '</h3><p>' + tempCelsius + " : " + weatherDesc + '</p>'))
    .addTo(map);
});
...
map.on('load', function() {
  ...
  map.addLayer({
    ...
    "maxzoom": 9,
    "paint": {
      // Increase the heatmap weight based on frequency and property precipnitude
      "heatmap-weight": [
        "interpolate",
        ["linear"],
        ["get", "precip"],
